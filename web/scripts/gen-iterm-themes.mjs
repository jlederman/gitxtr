...
if (!existsSync(REPO_DIR)) {
    ...
    execSync(`git clone --depth 1 https://github.com/mbadolato/iTerm2-Color-Schemes ${REPO_DIR}`, {
        stdio: "inherit",
    });
} else {
...
function parsePlist(content) {
    ...
    while ((m = re.exec(content)) !== null) {
        ...
        const comp = (k) => {
            const r = new RegExp(
                `<key>${k}<\\/key>\\s*<(?:real|integer)>([\\d.e+\\-]+)<\\/(?:real|integer)>`,
            ).exec(body);
...
function toTheme(filename, colors) {
    ...
    const fg = get("Foreground Color", { r: 0.8, g: 0.8, b: 0.8 });
    ...
    const surface2 = isLight ? mix(bg, BLACK, 0.12) : mix(bg, BLACK, 0.3);
