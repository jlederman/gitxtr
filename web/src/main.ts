...
const remoteBtns = ["btn-fetch", "btn-pull", "btn-push"].map(
...
async function boot(): Promise<void> {
    ...
    let curDetailTopHeight = Math.max(
        TOP_MIN,
        Math.min(curDetailHeight - DIFF_RESERVE, settings.detailTopHeight),
    );
    let curDetailMetaHeight = Math.max(
        META_MIN,
        Math.min(curDetailTopHeight - FILE_RESERVE, settings.detailMetaHeight),
    );
    ...
    initSplitter({
        ...
        onResize: (px) => {
            curDetailMetaHeight = px;
            applyDetailMetaHeight(px);
        },
    ...
    initDiffToolbar(
        settings.diffView,
        (m) => void request("saveSettings", { settings: { diffView: m } }),
    );
    initFileNav();
    initCommitModal();
    initRebaseModal();
    ...
    remoteBtns.forEach((btn, i) =>
        btn.addEventListener("click", () => {
            const repo = getCurrentRepo();
            if (repo)
                openRemoteModal(modes[i], {
                    repo,
                    branches: branchList,
                    onDone: () => void loadGraph(repo),
                });
        }),
    );
    initFileHistory((sha) => {
        if (!renderer.selectBySha(sha))
    ...
    initContextMenu((action, payload) => {
        ...
        if (p.kind === "commit") {
            ...
            if (action === "copy-sha") void navigator.clipboard.writeText(row.shortSha);
                ...
                if (repo && confirm(`Delete branch "${name}"?`))
                    void branchOp(repo, "delete", { name });
                ...
                if (repo && name)
                    void branchOp(repo, "create", { name, sha: row.sha, checkout: true });
                ...
                if (repo) {
                    ...
                    if (!toRebase) {
                        alert(
                            `"${row.summary}" is not on the current branch's linear history and cannot be used as a rebase base.`,
                        );
                    } else {
    ...
    shortcutsEl.addEventListener("pointerdown", (e) => {
        if (e.target === shortcutsEl) shortcutsEl.hidden = true;
    });
...
function textMatchesSha(q: string, row: Row): boolean {
    return (
        row.sha.toLowerCase().startsWith(q) ||
...
function applyFilter(): void {
    ...
    if (!raw) {
        ...
        const display: GraphView =
            fullView.hasUncommittedChanges && viewMode !== "simple"
    ...
    if (filterHide) {
        const display: GraphView = {
            rows: fullView.rows
                .filter((r) => matchSet.has(r.sha))
                .map((r) => ({ ...r, column: 0, edges: [] })),
            width: 0,
...
async function applyPathFilter(repo: string, filePath: string): Promise<void> {
    ...
    try {
        const shas = await request<string[]>("getCommitsByPath", {
            repoPath: repo,
            path: filePath,
        });
...
async function loadBranches(repo: string): Promise<void> {
    try {
        const list = await request<
            { name: string; isHead: boolean; upstreamName?: string | null }[]
        >("getBranches", { repoPath: repo });
        branchList = list;
