...
export function initFileNav(): void {
    ...
    el.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown") {
            e.preventDefault();
            selectFile(Math.min(totalFiles - 1, selectedFileIdx + 1));
        } else if (e.key === "ArrowUp") {
            e.preventDefault();
            selectFile(Math.max(0, selectedFileIdx - 1));
        }
    });
}
...
function selectFile(idx: number): void {
    selectedFileIdx = idx;
    filesEl()
        .querySelectorAll<HTMLElement>(".fitem[data-idx]")
        .forEach((el) => el.classList.toggle("sel", Number(el.dataset.idx) === idx));
    filesEl()
        .querySelector<HTMLElement>(`.fitem[data-idx="${idx}"]`)
...
function renderPatchString(patch: string): void {
    const html =
        diffMode === "split"
            ? renderSplit(patch)
            : `<div class="diff-wrap">${renderUnified(patch)}</div>`;
...
async function loadDetails(sha: string, parent: ParentSel): Promise<void> {
    ...
    parentSel = parent;
    try {
        const d = await request<CommitDetails>("getCommitDetails", {
            sha,
...
function renderWipMeta(view: WorkingTreeView): void {
    ...
    metaEl().innerHTML =
        ...
        (s > 0
            ? `<div><span class="sha">● ${s} staged</span></div>`
            : `<div style="color:var(--muted)">nothing staged</div>`) +
...
function renderWipFiles(view: WorkingTreeView): void {
    ...
    if (view.staged.length > 0) {
        html +=
            `<div class="fhead">Staged changes` +
    ...
    if (view.unstaged.length > 0) {
        html +=
            `<div class="fhead">Unstaged changes` +
...
async function handleWipOp(op: string, filePath: string): Promise<void> {
    ...
    if (op === "discardAll" && !confirm("Discard all unstaged changes? This cannot be undone."))
        return;
...
function wipFileItem(f: WorkingTreeFile, idx: number): string {
    ...
    const ops = f.staged
        ...
        : `<span class="fops">` +
          ...
          `</span>`;
    return (
        `<div class="fitem" data-idx="${idx}" title="${esc(f.path)}">` +
...
function statusInfo(status: string): { glyph: string; cls: string } {
    switch (status) {
        case "Added":
            return { glyph: "A", cls: "st-add" };
        case "Deleted":
            return { glyph: "D", cls: "st-del" };
        case "Renamed":
            return { glyph: "R", cls: "st-ren" };
        case "Copied":
            return { glyph: "C", cls: "st-ren" };
        default:
            return { glyph: "M", cls: "st-mod" };
    }
}
    ...
    filesEl().innerHTML =
        ...
        d.files
            .map((f, i) => {
                ...
                return (
                    `<div class="fitem" data-idx="${i}" title="${esc(f.path)}">` +
...
function wordDiff(oldText: string, newText: string): [string, string] {
    ...
    while (i < m && j < n) {
        if (a[i] === b[j]) {
            ...
            oldHtml += `<span class="wd-del">${esc(a[i++])}</span>`;
        } else {
            newHtml += `<span class="wd-add">${esc(b[j++])}</span>`;
        }
    }
...
export function renderUnified(diff: string, truncated = false): string {
    ...
    for (const line of lines) {
        if (line.startsWith("diff --")) {
            flush();
            out.push(`<div class="line fhdr" id="diffsec-${++sec}">${esc(line) || "&nbsp;"}</div>`);
        } else if (line.startsWith("+++") || line.startsWith("---") || line.startsWith("index ")) {
            flush();
            out.push(`<div class="line fhdr">${esc(line) || "&nbsp;"}</div>`);
        } else if (line.startsWith("@@")) {
            flush();
            out.push(`<div class="line hunk">${esc(line) || "&nbsp;"}</div>`);
        } else if (line.startsWith("+")) adds.push(line.slice(1));
        else if (line.startsWith("-")) dels.push(line.slice(1));
        else {
            flush();
            out.push(`<div class="line">${esc(line) || "&nbsp;"}</div>`);
        }
    }
    flush();
...
function srow(lno: number | null, html: string, cls: string): string {
    return (
        `<div class="srow2"><span class="lno">${lno ?? ""}</span>` +
