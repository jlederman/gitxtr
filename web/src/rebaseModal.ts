...
interface RebaseStep { sha: string; action: string; shortSha: string; summary: string; }
...
export function initRebaseModal(): void {
  ...
  rebaseBtn().addEventListener("click", () => { errEl().textContent = ""; void execute(); });
...
export function openRebaseModal(rows: Row[], repo: string): void {
  ...
  repoPath = repo;
  steps = rows.map(r => ({ sha: r.sha, action: "pick", shortSha: r.shortSha, summary: r.summary }));
  render();
...
function render(): void {
  ...
  listEl().innerHTML = steps.map((s, i) => {
    ...
    return `<div class="ritem${drop ? " r-drop" : ""}" data-idx="${i}">` +
      `<div class="r-arrows">` +
  ...
  listEl().onclick = (e) => {
    ...
    if (up) { const i = parseInt(up.dataset.idx!); if (i > 0) { swap(i, i - 1); render(); } }
    if (dn) { const i = parseInt(dn.dataset.idx!); if (i < steps.length - 1) { swap(i, i + 1); render(); } }
  };
