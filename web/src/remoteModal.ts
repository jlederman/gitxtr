...
interface RemoteDto { name: string; url: string; }
...
export function openRemoteModal(m: RemoteMode, c: RemoteContext): void {
  ...
  const merge = document.querySelector<HTMLInputElement>('input[name="rf-pullmode"][value="merge"]');
...
function populateBranches(current?: string): void {
  ...
  sel.innerHTML = (ctx?.branches ?? []).map(b =>
...
async function execute(): Promise<void> {
  ...
  if (mode === "pull")
    payload.rebase = document.querySelector<HTMLInputElement>('input[name="rf-pullmode"]:checked')?.value === "rebase";
  if (mode === "push") {
