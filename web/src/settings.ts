...
interface GitIdentity {
  globalName: string | null; globalEmail: string | null;
  localName: string | null; localEmail: string | null;
}
...
export function initSettings(opts: {
  ...
  const onAppearance = () => {
    ...
    void request("saveSettings", {
      settings: { theme: current.theme, fontFamily: current.fontFamily, fontSize: current.fontSize },
    });
  };
...
function selectCat(cat: string): void {
  document.querySelectorAll<HTMLElement>("#settings-cats button")
...
async function saveGitIdentity(): Promise<void> {
  ...
  try {
    ...
    const globalChanged = gn !== (loadedIdentity?.globalName ?? "") || ge !== (loadedIdentity?.globalEmail ?? "");
    const localChanged = ln !== (loadedIdentity?.localName ?? "") || le !== (loadedIdentity?.localEmail ?? "");
    if (globalChanged) await request("setGitIdentity", { scope: "global", name: gn, email: ge, ...repoArg });
    if (localChanged) await request("setGitIdentity", { scope: "local", name: ln, email: le, ...repoArg });
...
function checkFontStatus(): void {
  ...
  $("appearance-status").textContent = isFontAvailable(fam) ? "" : `Font “${first}” not found — using fallback`;
}

const GENERIC_FONTS = new Set([
  "serif", "sans-serif", "monospace", "cursive", "fantasy", "system-ui",
  "ui-monospace", "ui-sans-serif", "ui-serif", "ui-rounded", "math", "emoji", "fangsong",
]);
