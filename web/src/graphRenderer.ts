...
export class GraphRenderer {
    ...
    private onKey(e: KeyboardEvent): void {
        ...
        switch (e.key) {
            ...
            case "c":
                if (this.selected >= 0)
                    void navigator.clipboard.writeText(this.view.rows[this.selected].shortSha);
                return;
            case "C":
                if (this.selected >= 0)
                    void navigator.clipboard.writeText(this.view.rows[this.selected].sha);
                return;
            default:
                return;
        }
    ...
    private onPointerDown(e: PointerEvent): void {
        ...
        if (h && e.offsetY >= this.vh - SCROLLBAR_W) {
            this.dragging = "h";
            this.dragOffset =
                e.offsetX >= h.x && e.offsetX <= h.x + h.w ? e.offsetX - h.x : h.w / 2;
        ...
        if (v && e.offsetX >= this.vw - SCROLLBAR_W) {
            this.dragging = "v";
            this.dragOffset =
                e.offsetY >= v.y && e.offsetY <= v.y + v.h ? e.offsetY - v.y : v.h / 2;
    ...
    private dragVTo(offsetY: number): void {
        ...
        const frac =
            travel > 0 ? Math.max(0, Math.min(1, (offsetY - this.dragOffset) / travel)) : 0;
    ...
    private dragHTo(offsetX: number): void {
        ...
        const frac =
            travel > 0 ? Math.max(0, Math.min(1, (offsetX - this.dragOffset) / travel)) : 0;
