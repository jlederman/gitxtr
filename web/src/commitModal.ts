...
export function initCommitModal(): void {
  ...
  commitBtn().addEventListener("click", () => { errSpan().textContent = ""; void performCommit(); });
...
async function reload(repo: string): Promise<void> {
  try {
    ...
    if (selectedPath) {
      ...
      const f = all.find(f => f.path === selectedPath);
      if (f) showDiff(f);
      else { selectedPath = null; diffPane().innerHTML = ""; }
    }
  } catch (e) {
    filesPane().innerHTML =
      `<div style="padding:12px;color:var(--del-fg)">Error: ${esc(e instanceof Error ? e.message : String(e))}</div>`;
  }
}
...
function renderFiles(view: WorkingTreeView): void {
  ...
  el.onclick = (e) => {
    ...
    if (btn) { e.stopPropagation(); void handleOp(btn.dataset.op!, btn.dataset.path ?? ""); return; }
...
function citem(f: WorkingTreeFile): string {
  ...
  const ops = f.staged
    ...
    : `<span class="fops">` +
      ...
      `</span>`;
  return `<div class="citem${sel}" data-path="${esc(f.path)}" title="${esc(f.path)}">` +
