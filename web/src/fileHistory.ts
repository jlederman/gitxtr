...
export function initFileHistory(navigate: (sha: string) => void): void {
  ...
  listEl().addEventListener("dblclick", (e) => {
    const row = (e.target as HTMLElement).closest<HTMLElement>(".fh-row[data-sha]");
    if (row?.dataset.sha) { select(row.dataset.sha); gotoGraph(); }
  });
  listEl().addEventListener("keydown", (e) => {
    if (commits.length === 0) return;
    const i = commits.findIndex(c => c.sha === selectedSha);
    if (e.key === "ArrowDown") { e.preventDefault(); select(commits[Math.min(commits.length - 1, i + 1)].sha); }
    else if (e.key === "ArrowUp") { e.preventDefault(); select(commits[Math.max(0, i - 1)].sha); }
    else if (e.key === "Enter") { e.preventDefault(); gotoGraph(); }
  });
...
function gotoGraph(): void {
  if (selectedSha) { onNavigate(selectedSha); close(); }
}
...
function syncTabs(): void {
  document.querySelectorAll<HTMLElement>("#fh-tabs button").forEach(b =>
    b.classList.toggle("active", b.dataset.tab === mode));
  hintEl().textContent = mode === "blame"
...
function select(sha: string): void {
  ...
  selectedSha = sha;
  listEl().querySelectorAll<HTMLElement>(".fh-row[data-sha]").forEach(r =>
    r.classList.toggle("sel", r.dataset.sha === sha));
  listEl().querySelector<HTMLElement>(`.fh-row[data-sha="${CSS.escape(sha)}"]`)?.scrollIntoView({ block: "nearest" });
  void renderDetail();
}
...
async function renderDetail(): Promise<void> {
  ...
  try {
    if (mode === "history") {
      ...
      if (diff === undefined) {
        const d = await request<{ diff: string }>("getCommitDetails", { repoPath: repo, sha });
      ...
      if (blame === undefined) {
        blame = await request<FileBlame>("getBlame", { repoPath: repo, path: currentPath, sha });
